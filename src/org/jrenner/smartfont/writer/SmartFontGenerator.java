...
public class SmartFontGenerator {
	 ...
	 */
	public BitmapFont createFont(FileHandle fontFile, String fontName,
			int fontSize) {
		...
		if (displayWidth != Gdx.graphics.getWidth()
			...
			Gdx.app.debug(TAG,
					"Screen size change detected, regenerating fonts");
		} else {
			try {
				...
				Gdx.app.debug(TAG,
						"Couldn't load pre-generated fonts. Will generate fonts.");
			}
		}
		if (!loaded || forceGeneration) {
			...
			font = generateFontWriteFiles(fontName, fontFile, fontSize,
					pageSize, pageSize);
		}
		return font;
	}
	 ...
	 * @param fontSize
	 */
	private BitmapFont generateFontWriteFiles(String fontName,
			FileHandle fontFile, int fontSize, int pageWidth, int pageHeight) {
		...
		FreeTypeFontGenerator.FreeTypeBitmapFontData fontData = generator
				.generateData(fontSize, FreeTypeFontGenerator.DEFAULT_CHARS,
						false, packer);
		Array<PixmapPacker.Page> pages = packer.getPages();
		TextureRegion[] texRegions = new TextureRegion[pages.size];
		for (int i = 0; i < pages.size; i++) {
			PixmapPacker.Page p = pages.get(i);
			Texture tex = new Texture(new PixmapTextureData(p.getPixmap(), p
					.getPixmap().getFormat(), false, false, true)) {
				@Override
				public void dispose() {
					super.dispose();
	 ...
	 * unaltered when creating a font. If the screen width is 720, the font size
